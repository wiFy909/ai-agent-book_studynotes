...
模型选型不只是在比较“能不能做成”，还要比较模型**默认会怎样做**。Coding Agent 中一个很容易观察到的差异是行动阈值：面对同一个代码任务，有的模型会先广泛探索仓库，确认架构、调用方和测试之后才修改；有的模型则凭较少的局部证据快速定位，先改再用测试反馈补齐认识。前者把过早修改的风险估得更高，后者把“再读一个文件”的机会成本估得更高。

当这种倾向在更换 Harness 后仍跟随模型，并且在固定 Harness 中仅替换模型就发生变化时，主要解释就应落在**模型行为**上。后训练很可能是关键来源：SFT 轨迹会示范“先读到什么程度再动手”，过程奖励会奖励或惩罚某种工具路径，结果奖励又会强化最终成功的整套策略。久而久之，模型学到的不只是如何写代码，也包括何时已经掌握足够证据。具体训练数据和奖励配方通常不公开，所以可以从受控换模实验判断行为位于模型侧，却不能反推出某家公司的精确训练秘方。Harness 仍可通过系统提示、工具描述和预算改变阈值，但在没有强制流程时，它只是调节因素，不应被默认当作根因。

配套实验在一个**中性、固定的 Harness** 中比较 `openai/gpt-5.6-sol` 与 `anthropic/claude-sonnet-5`：两者经同一个 OpenRouter 端点接入，看到相同的系统提示、任务、仓库、工具名称与 JSON Schema，Harness 不要求“先探索”或“立即修改”。三个小型代码库分别覆盖局部 bug、跨模块身份规范化和公共契约敏感的缓存修复，每个模型对每题独立运行三次，共 18 条轨迹。结果中，GPT-5.6-sol 在第一次修改前平均调用工具 6.89 次、读取 4.67 个文件；Claude Sonnet 5 分别为 4.56 次和 3.56 个文件。差距在局部任务中最明显，而在明确跨模块的任务中几乎收敛（7.00 对 6.67 个文件）。两者的首个受测补丁和最终测试均为 100% 通过，因此这个小实验支持的是“行动策略随模型而变”，而不是“多读或早改必然更好”。时间到首次修改也几乎相同（15.01 秒对 14.48 秒），提醒我们工具步数、并行调用和模型延迟必须分开看。

> **实验 6-7 ★★：在固定 Coding Harness 中测量模型的行动阈值**
>
> **实验目标**：隔离模型因素，量化不同 Coding 模型在“继续收集信息”与“开始修改代码”之间的默认取舍，并把路径效率与最终质量联合起来评价。
>
> **技术方案**：运行 `chapter6/model-action-threshold/experiment.py`。默认通过同一个 OpenRouter OpenAI-compatible 端点调用 GPT-5.6-sol 与 Claude Sonnet 5，固定系统提示、工具 Schema、任务仓库、测试命令和最大轮次；中性提示既不要求读够若干文件，也不要求尽快编辑。三类任务各重复至少三次，并交替模型运行顺序。记录首次修改前的工具调用数、读文件数、搜索次数和墙钟时间，以及首个受测补丁通过率、测试后返工次数、最终成功率、改动文件数和 Token 用量。
>
...
>
> **验收标准**：离线单元测试全部通过；每个任务的初始仓库必须先被确认处于测试失败状态；正式结果包含完整的 `模型 × 任务 × 重复次数` 单元、零 API 错误、独立最终测试和可复核的轨迹；`manifest.json` 必须校验配置、逐条观察和汇总文件的哈希。配套目录已保存一轮 18/18 单元的完整实测结果，读者应在自己关心的模型版本和真实工作负载上重跑，而不能把这组小仓库的数值当成永久排行榜。
>
...
> **实验 6-12 ★★：配置 OpenVLA 与 RoboTwin2 的具身智能环境**
>
