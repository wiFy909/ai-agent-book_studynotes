...
이 루브릭을 에이전트의 실제 응답과 함께 평가 모델에 보내면 각 차원을 채점하고 근거를 제시합니다. 수십 개의 테스트 케이스에서 이를 실행하면 에이전트의 역량 격차를 체계적으로 찾을 수 있습니다. 예를 들어 "세션 간 연관" 차원의 평균 점수가 2.1점이라면 메모리 검색이나 정보 연관 기능에 결함이 있음을 분명히 가리킵니다.

> **실험 6-3 ★★: 루브릭 기반 사용자 메모리 평가 시스템 구축하기**
>
> **선행 조건**: 3장의 사용자 메모리 실험(`ch3/user-memory-evaluation`)을 완료해야 합니다.
>
> 이 실험에서는 3장의 `ch3/user-memory-evaluation` 프레임워크를 수정하여 현재의 단순한 LLM-as-a-Judge 채점 메커니즘을 구조화된 다차원 루브릭 평가 시스템으로 업그레이드합니다. 기존 시스템은 한 번의 LLM 호출로 통과/실패 결과와 평가 근거를 반환하므로 구조화된 진단 기능이 부족합니다.
>
...
>
> **선행 조건**: 3장의 사용자 메모리와 RAG 실험(`ch3/user-memory`, `ch3/agentic-rag-for-user-memory`)을 완료해야 합니다.
>
> **목표**: 같은 평가 세트에서 구조화된 메모리와 비구조화 검색의 장점과 경계를 공정하게 비교합니다. 3장의 두 프로젝트를 재사용하고 `ch3/user-memory-evaluation`의 테스트 케이스 60개에서 세 구성을 비교합니다. 순수 고급 JSON 카드(검색 없이 구조화된 카드를 컨텍스트에 유지), 순수 RAG(대화 청크를 벡터 저장소에 임베딩하고 검색 필요), 하이브리드 시스템(핵심 사실은 상주 + 원본 대화는 필요할 때 검색)입니다.
>
...
>
> 길이(한 문장 → 긴 단락), 장르(뉴스·이야기·대화), 감정(중립·흥분·슬픔), 특별한 과제(숫자·고유명사·다음자·방언 어휘)가 다양한 테스트 말뭉치를 구축하세요. 평가 파이프라인을 구현합니다. TTS 생성 모듈은 주류 서비스(OpenAI, ElevenLabs, Fish Audio, Minimax, Doubao)에 연결하고, 멀티모달 평가 모듈은 Gemini 3.5 Flash를 사용합니다. 합성 음성, 원문 텍스트, 참조 음성, 루브릭을 함께 제공하여 각 차원을 채점하고 상세한 근거를 제시하게 합니다. 평가 결과의 분포를 분석해 TTS 모델마다 차원별 강점과 약점을 찾으세요. 어떤 모델은 정확성이 뛰어나지만 자연스러움이 부족할 수 있고, 다른 모델은 자연스럽지만 특수 어휘에서 오류를 잘 낼 수 있습니다.
>

루브릭을 사람이 직접 정의하는 것 외에도 전문 **생성형 보상 모델**을 학습하여 판단을 자동화할 수 있습니다. 이는 보상 모델의 학습 방법과 관련되며 7장에서 자세히 논의합니다.
...
구체적인 사례로 비용의 비선형 증가를 살펴보겠습니다. 표 6-4는 이 장 앞부분의 고객 서비스 환불 에이전트를 예로 들고, 예시 토큰 가격 매개변수를 사용해 세 라운드의 호출 비용을 분해합니다. 여러 라운드의 컨텍스트 누적과 캐시 적중이 비용에 미치는 영향을 보여 줍니다.

표 6-4 고객 서비스 환불 에이전트의 3라운드 비용 사례

| 라운드 | 작업 | 입력 토큰 | 출력 토큰 | 라운드 비용 |
|-------|--------------------------------------------|------------------------|------------|---------|
| 1 | 시스템 프롬프트 + 사용자 질문 → 주문 조회 결정 | 2,500(시스템 프롬프트 2,000) | 150 | $0.0098 |
| 2 | 이전 라운드 컨텍스트 + 도구 결과 → 환불 접수 여부 결정 | 3,200(캐시 적중 2,000) | 120 | $0.0060 |
| 3 | 이전 라운드 컨텍스트 + 환불 결과 → 사용자에게 답변 | 3,800(캐시 적중 3,200) | 200 | $0.0058 |
| **합계** | | **9,500** | **470** | **$0.022** |

참고: 입력 토큰 100만 개당 $3, 출력 토큰 100만 개당 $15라는 예시 가격으로 계산했습니다. 캐시 적중 부분은 입력 가격의 10%로 청구된다고 가정했습니다(할인율은 공급자마다 다릅니다. 예를 들어 Anthropic의 캐시 쓰기는 입력 가격의 약 1.25배이고 캐시 읽기는 약 0.1배이지만, 여기서는 읽기 할인만으로 단순화했습니다).

3라운드에 $0.022라면 저렴해 보입니다. 캐시가 전혀 없다면 입력 비용만 약 $0.029이고 출력을 포함하면 약 $0.036입니다. 여기서는 캐싱으로 입력 비용을 거의 절반 절감했으며, 뒤에서 인용하는 경험적 범위("KV 캐시로 입력 비용을 30~60% 줄일 수 있음")와 일치합니다. 하지만 증폭 요인에 주의해야 합니다. 사고 모드를 활성화해 라운드마다 사고 토큰 500~2,000개를 추가로 출력하면 비용이 3~5배로 늘 수 있습니다. 도구 하나가 5,000토큰짜리 웹 페이지를 반환하면 이후 라운드마다 그 토큰 비용을 다시 지불합니다. 에이전트가 우회하여 10라운드가 필요해지면 컨텍스트가 20,000토큰을 훌쩍 넘어 이 단순한 시나리오와 비교할 수 없게 커집니다. 따라서 비용 최적화의 핵심은 더 저렴한 모델을 고르는 것이 아니라 라운드 수와 컨텍스트 증가를 제어하는 것입니다.

**비용 최적화 전략.**

정량적 관점에서 입력 측면에 가장 효과적인 수단은 **KV 캐시 재사용**(안정적인 접두사를 유지하여 반복되는 시스템 프롬프트, 도구 정의, 이전 라운드에 캐시 가격을 적용하고 입력 토큰 비용을 30~60% 절감. 위 3라운드 사례에서는 캐싱으로 입력 비용을 거의 절반 줄임), **컨텍스트 압축**(과거 궤적을 압축하고 중복 도구 출력을 잘라내 컨텍스트 증가율을 직접 제어하며 장기 작업에서 특히 효과적), **계층형 모델 라우팅**(단순한 요청은 경량 모델, 복잡한 사고는 강력한 모델)입니다. 접두사 안정성 설계, 압축 시점과 전략, 라우팅 메커니즘이라는 세 방법의 구체적인 구현은 2장에서 자세히 설명했으므로 여기서는 반복하지 않습니다. 이 장에서는 평가·운영 관점의 두 방법을 보충합니다.
...
>
> **실험 목표**: 대표적인 에이전트 작업의 전체 비용을 분해하여 비용 기준선을 수립하고 최적화 전략의 효과를 검증합니다.
>
> **기술적 접근법**: 대표 작업 몇 가지를 고르고 LangSmith 또는 자체 구축한 트레이싱 시스템으로 각 LLM 호출의 입력·출력 토큰 수, 사고 토큰 수, 도구 호출 수와 반환 크기, 엔드투엔드 지연 시간을 기록합니다. 작업 유형별 평균 비용, 비용 분포(p50/p95/p99), 비용 구성 비율을 계산합니다.
>
> **인수 기준**: 비용 내역 보고서를 생성하고 주요 비용 요인을 식별합니다. KV 캐시와 컨텍스트 압축을 각각 활성화·비활성화했을 때 비용 차이를 비교합니다.
>
>
...
>
> **목표**: 사용자 메모리 검색 에이전트를 엔드투엔드로 모델 선택 평가하여 임베딩 모델, 재순위화 모델, 에이전트의 주 모델이 검색 품질·지연 시간·비용에 함께 미치는 영향을 살펴봅니다. `ch3/contextual-retrieval-for-user-memory` 또는 `ch3/agentic-rag-for-user-memory`를 재사용하고 테스트 케이스 60개에서 각 구성을 비교합니다.
>
...
에이전트 평가에는 또 다른 비결정성도 있습니다. 같은 모델과 데이터셋을 사용해도 두 번의 실행 결과가 달라질 수 있습니다. 온도 샘플링, 변동하는 도구 반환값, 환경의 타이밍이 모두 무작위성을 더합니다. 따라서 한 번의 실행 수치만으로 결정을 내려서는 안 됩니다. **여러 번 실행하고 평균을 내며**(예: 설정당 3~5회) 평균과 분산을 함께 보고해야 합니다. 뒤의 가상 사례에서 모든 설정을 "5회 실행(서로 다른 무작위 시드 사용)"하는 이유도 바로 이것입니다.
...
쉽게 간과하는 함정이 하나 더 있습니다. 바로 **다중 비교**입니다. 여러 가설을 병렬로 테스트하면 결론 중 하나 이상이 거짓 양성일 확률이 빠르게 올라갑니다. 결론마다 신뢰 수준이 95%여도 가설 6개에서는 거짓 양성이 하나 이상 나올 확률이 1 − 0.95^6 ≈ 26%입니다. 병렬로 실행하는 가설이 많을수록 유의미해 보일 뿐인 결과 하나를 피하기 어려워집니다. 대응책은 두 가지입니다. 가설 수가 늘어날수록 각 결론의 유의성 임계값을 더 엄격하게 조정하거나(Bonferroni 방식의 교정), 긍정적인 결과를 독립적인 확인 단계에서 다시 실행하여 재현될 때만 믿습니다. 뒤의 "데이터에서 가설로" 절에서는 실제로 병렬인 네 가설 H1~H4를 테스트합니다(H5와 H6은 조건부로 촉발되며 앞의 네 가설과 동시에 실행하지 않습니다). 이는 이 함정의 전형적인 시나리오입니다.
...
**다음은 가상의 교육용 사례**입니다. 구체적인 데이터로 벤치마크 보고서에서 시스템 개선에 이르는 완전한 의사결정 과정을 설명합니다. 데이터는 가상이며 실제 실험 결과를 보고하는 것이 아니라 방법론을 보여 주기 위한 것입니다.
...
구체적인 사례로 벤치마크 보고서를 읽는 방법을 살펴보겠습니다. AndroidWorld에서 에이전트를 평가하여 작업별 성능 표와 역량 태그 성능 행렬이라는 두 핵심 보고서 표를 얻었다고 가정합니다. 보고서의 가치는 전체 성공률이라는 단일 수치가 아니라 구조적인 약점을 드러내는 데 있습니다.

작업별 표에는 뚜렷한 패턴이 나타납니다. 대부분의 일상적인 작업은 성공률이 100%에 가깝습니다. 녹음, 사진 촬영, 연락처 관리, 메모 작성, 파일 작업, 시스템 설정 같은 흔한 시나리오로, 평균적으로 10단계가 넘고 가장 복잡한 것은 수십 단계가 필요합니다. 이처럼 긴 행동 순서를 성공적으로 완료한다는 것은 표준 시나리오에서 에이전트의 계획·실행 능력을 보여 줍니다.

실패는 SMS 답장, Wi-Fi 전환과 상태 확인, 할 일 목록 조회, Wi-Fi+Bluetooth 결합 작업, VLC 재생 목록 생성이라는 몇 영역에 밀집합니다. 표면적으로 서로 무관해 보이지만 역량 태그 행렬을 보면 공통점이 드러납니다.

**역량 태그 행렬**은 진단의 핵심입니다. 필요한 역량과 난이도를 기준으로 모든 작업을 교차 분류합니다. 보고서에서는 흔히 몇 가지 역량 차원의 성공률이 매우 낮게 나타납니다. transcription(이미지·동영상에서 정보를 옮겨 적는 능력으로 시각 이해의 결함을 드러냄), math_counting(수학 능력 자체가 문제가 아닙니다. 현대 LLM은 수학에 강하지만, 계산이 필요하다는 사실을 인식하고 UI에서 숫자를 추출해 결과를 행동 순서에 매핑할 수 있는지가 문제입니다), complex_ui_understanding(표준 UI 패턴에 크게 의존하여 비표준 레이아웃을 만나면 무너짐)입니다.

두 표를 함께 읽으면 실패가 설명됩니다. 할 일 조회 실패는 에이전트가 읽고 필터링할 수 없는 비표준 UI에서 비롯됩니다. Wi-Fi 실패는 에이전트의 이해 범위를 넘어서는 시스템 설정의 제어 계층에서 발생합니다. VLC 재생 목록 실패는 전문 애플리케이션의 복잡한 UI에서 생성 진입점을 찾지 못하기 때문입니다.

### 데이터에서 가설로: 개선 로드맵 구축

**표면 수준 가설**(비용이 낮고 독립적이며 병렬 검증 가능): H1은 Wi-Fi 작업에 시스템 설정 탐색 힌트를 추가하는 것입니다. 에이전트가 스위치는 조작할 수 있지만 진입 페이지를 찾지 못하는 것일 수 있으므로 설정 관련 작업에 밀집된 실패를 해결할 것으로 예상합니다. H2는 할 일 앱의 UI 요소 식별 규칙을 제공하여 할 일 작업의 실패를 해결할 것으로 예상합니다.

**중간 수준 가설**(역시 독립적이며 병렬화 가능): H3은 멀티모달 입력 파이프라인을 수정하는 것입니다. 실패한 트레이스를 재생하면 파이프라인에서 이미지가 누락되거나 텍스트 설명으로 변환되어 가장 강력한 멀티모달 모델조차 정보를 옮겨 적을 수 없었을 가능성이 드러납니다. H4는 사고를 전역적으로 활성화하여 수량 집계 관련 실패를 해결하는 것입니다.

**심층 수준 가설**(검증 비용이 높으며 표면·중간 수준 개선 후에도 complex_ui 성공률이 40% 미만일 때만 시작): H5는 시각 이해가 더 강한 모델(GPT-5)로 교체하는 것입니다. H6는 스크린샷 외에 UI 요소 트리 정보(UI Automator로 추출한 구조화된 DOM)를 추가하여 스크린샷과 교차 검증하는 것입니다. 두 가설로 2×2 비교 실험(Claude/GPT-5 × 스크린샷만/스크린샷 + 요소 트리)을 구성하여 "모델 역량과 정보의 풍부함 중 어느 것이 더 중요한가, 시너지 효과가 있는가?"에 답할 수 있습니다.

각 설정은 작업 116개 전체에서 5번씩 실행하며(서로 다른 무작위 시드 사용), 성공률, 평균 단계 수, 실행 시간을 기록합니다.

### 결과에서 의사결정으로: 데이터 기반 절충

실험에서 다음과 같은 결과가 나왔다고 가정합니다(**아래 데이터는 모두 가상입니다**). H1은 입력 토큰이 8% 늘어나는 대신 설정 관련 작업의 성공률을 0%에서 75%로 높입니다. H3는 시각 토큰이 15% 늘고 단계별 지연 시간이 1초 증가하지만 transcription을 0%에서 80%로 개선합니다. H4는 counting을 0%에서 70%로 개선하지만 단계별 지연 시간이 4초에서 12초로 늘고 비용이 3배가 됩니다. H6는 토큰이 30% 늘고 단계별 지연 시간이 2초 증가하지만 complex_ui를 17%에서 52%로 개선합니다. H5(GPT-5)는 complex_ui를 17%에서 35%로 개선하지만 단계별 지연 시간이 4초에서 15초로 늘어납니다.

의사결정은 효과가 있는 개선을 모두 채택하는 것처럼 단순하지 않습니다.

**H1과 H3는 즉시 배포합니다.** H1은 부작용 없이 비용은 낮고 이점은 큽니다. H3는 시각 토큰 비용을 15% 늘리고 지연 시간을 1초 추가하지만, 작동하지 않던 transcription 역량을 작동하게 만들며 멀티모달 정보를 누락하는 입력 파이프라인의 아키텍처 결함을 수정합니다. 그 과정에서 다른 시각 이해 작업도 향상될 수 있습니다.

**H4를 전역적으로 활성화하는 것은 받아들일 수 없습니다.** 전체 성공률은 88%에서 91%로 오르지만 역량 태그 분포를 보면 counting이 필요한 작업은 약 8%뿐입니다. 소수 작업을 위해 모든 작업의 지연 시간과 비용을 3배로 만드는 것은 닭 잡는 데 소 잡는 칼을 쓰는 전형적인 사례입니다. 하지만 H4는 counting 작업에 사고가 효과가 있음을 입증하여 다음 라운드의 조건부 활성화를 위한 토대를 마련합니다.

**H6가 H5보다 낫습니다.** H5(GPT-5)는 단계별 지연 시간을 4초에서 15초로 급증시키지만 complex_ui는 35%에 그칩니다. 병목은 모델의 사고가 아니라 입력에 충분한 정보가 담겼는지입니다. H6(요소 트리 추가)는 토큰 30% 증가와 지연 시간 2초를 대가로 35%포인트를 개선하여 훨씬 효율적입니다. H5+H6 조합은 가장 높은 68%를 기록하지만 대규모로 사용할 때 작업 시간이 받아들일 수 없는 수준입니다. 은행 송금이나 의료 예약 같은 중요한 비동기 작업에 선택적으로 활성화하기에는 적합하지만 일상적인 시나리오에는 H6로 충분합니다.

**H2는 확장할 수 없습니다.** 비표준 애플리케이션마다 전용 규칙을 작성하는 방식은 지속 가능하지 않습니다. 임시방편으로만 사용할 수 있으며 장기적인 해법은 에이전트의 일반화 능력을 높이는 것입니다.

### 지속적 개선: 첫 개선에서 시스템 진화까지

H1, H3, H6이라는 세 개선을 구현하고(H4는 배포하지 않음) 나면 AndroidWorld에서 에이전트의 성공률이 88%에서 94%로 올라갑니다. 전체 벤치마크를 다시 실행하면 새 보고서에는 다른 실패 패턴이 나타납니다. transcription, 설정, 복잡한 UI 작업은 모두 크게 개선됩니다. 약 6%인 남은 실패율은 해결되지 않은 counting 작업, 불안정한 Wi-Fi 상태 확인(0%에서 60%로 올랐지만 여전히 불안정), 새로 생긴 소수의 실패에 집중됩니다. 새 실패는 더 길어진 프롬프트나 지나치게 많은 요소 트리 정보가 모델을 방해해서 발생했을 수 있습니다.

새 보고서와 H4 실험의 통찰을 바탕으로 새로운 가설을 세울 수 있습니다. H7은 사고의 조건부 활성화입니다. 작업을 시작하기 전에 빠른 LLM 호출(약 1~2초)로 작업 설명을 분석하여 수량 집계나 복잡한 사고가 필요한 작업에서만 사고 모드를 활성화하고, 지연 시간 증가를 정말 필요한 작업으로 제한합니다. H8은 핀치 투 줌, 길게 눌러 드래그, 멀티터치 같은 복잡한 제스처를 지원하도록 행동 공간을 확장하는 것입니다. 남은 실패 트레이스를 재생하면 일부 작업에는 지도 확대, 이미지 자르기, 목록에서 길게 눌러 메뉴 열기 같은 조작이 필요하다는 사실이 드러납니다.

벤치마크 피드백을 바탕으로 이렇게 반복하면 에이전트의 역량이 꾸준히 향상됩니다. 벤치마크는 일회성 시험이 아니라 지속적인 건강 검진입니다. 정기적인 평가 주기(예: 매주 전체 테스트 스위트 실행)를 두면 역량 곡선을 관찰하고, 회귀(새 기능이 버그를 도입)를 일찍 포착하며, 개선(최적화가 실제로 효과가 있었음)을 확인하고, 어떤 종류의 개선은 대개 효과가 있고 어떤 것은 역효과가 나는지 지식을 축적할 수 있습니다. 데이터 기반, 가설 검증, 지속적 반복이라는 이 방법론은 경험 중심 에이전트 엔지니어링에서 과학적 엔지니어링으로 나아가는 핵심 경로입니다.

> **실험 6-10 ★★★: AndroidWorld에서 평가하고 개선하기**
>
> 이 실험은 평가 보고서에서 시스템 개선까지 이어지는 완전한 폐루프 실습입니다. `ch6/android-world`의 AndroidWorld 평가 보고서에서 시작합니다.
>
...
>
> 3단계: 단계적 실험. 가설을 테스트하는 통제 실험을 설계합니다. 1단계에서는 프롬프트 최적화와 더 풍부한 도구 설명 같은 저비용 표면 가설을 테스트합니다. 2단계에서는 중간 수준 역량 가설(입력 파이프라인 수정, 사고 모드 전환)을 테스트합니다. 특정 역량 태그와 연관된 작업의 개선 폭에 초점을 맞추면서 부작용도 측정합니다.
>
...
>
> 5단계: 반복. 개선을 완료한 뒤 데이터셋에서 평가를 다시 실행합니다. LLM으로 평가 결과를 분석하고 새 보고서를 생성합니다. 새 보고서는 달라진 실패 패턴을 보여 주며 다음 반복의 출발점이 됩니다.
>
...
이 장은 에이전트가 실제로 개선되었는지 어떻게 아는가라는 한 질문을 중심으로 전개했습니다. 재현 가능한 테스트 환경 구축, 유출에 견디는 데이터셋 설계, LLM을 평가자로 활용, 평가 결과에 따른 모델 선택과 반복 개선에 이르는 모든 연결 고리가 결론의 신뢰성에 영향을 줍니다. 모델 선택 시에도 단일 운영 지점에 의존하지 말고 리소스 예산에 따른 역량 성장 곡선을 비교해야 합니다. 프로덕션 수준 에이전트에서 평가는 가끔 치르는 시험이 아니라 모든 제품 의사결정에 내장된 지속적인 검증입니다.
...
7. ★ 이 장의 가상 사례에서는 사고를 전역적으로 활성화(H4)하여 전체 성공률이 높아졌지만 지연 시간과 비용 때문에 거부하고, 결국 조건부 활성화(H7)로 이어졌습니다. 어떤 신호(작업 설명 특징, 과거 실패 패턴, 런타임 불확실성)를 "사고 모드를 활성화할지" 결정하는 라우팅 기준으로 사용할 수 있을까요? 사고가 오히려 해로운 에이전트 시나리오도 있을까요?
