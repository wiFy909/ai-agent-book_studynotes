...
**Mem0: 추출–비교–결정의 2단계 파이프라인.** Mem0(Chhikara et al., 2025, arXiv:2504.19413)의 핵심은 두 단계로 실행되는 “추출–비교–결정” 메모리 파이프라인입니다(그림 3-3).


![그림 3-3 Mem0 메모리 관리 아키텍처](images/fig3-3.svg)


**추출 단계:** 새 대화 구간이 끝날 때마다 Mem0는 최근 대화와 기존 메모리 요약을 LLM에 전달하여 “사용자가 상하이로 이사했다” 같은 간결한 사실 문장인 후보 메모리 집합을 추출합니다. **갱신 단계:** 시스템은 후보 메모리마다 먼저 벡터 검색으로 의미가 비슷한 기존 메모리를 찾습니다. 그런 다음 LLM이 후보 메모리와 검색한 메모리의 관계를 비교하여 네 가지 결정 중 하나를 내립니다. **ADD**(완전히 새로운 정보를 바로 저장), **UPDATE**(기존 메모리를 보완하거나 수정), **DELETE**(새 정보가 이전 메모리와 모순되므로 이전 것을 삭제), **NOOP**(중복 정보이므로 아무 작업도 하지 않음)입니다. 사용자가 “상하이로 이사했어요”라고 하면 Mem0는 기존 메모리 “사용자는 베이징에 거주한다”를 검색해 UPDATE로 판단하고 “사용자는 상하이에 거주한다”로 갱신합니다. 모순되는 두 기록을 함께 남기지 않습니다. 이 파이프라인은 이 장 첫머리의 “선택적 추출”과 뒤에서 다룰 “충돌 해결”을 하나의 메커니즘으로 통합합니다. 메모리 저장소의 모든 기록은 기존 메모리와 명시적으로 조정된 뒤에 들어갑니다.

Mem0는 다양한 애플리케이션 요구에 적응할 수 있도록 모듈성이 높은 아키텍처를 사용합니다. 임베딩, 즉 텍스트를 벡터로 바꾸는 기능과 벡터의 지속적 저장 및 검색 기능을 분리하여 각각 독립적으로 최적화하고 교체할 수 있습니다. 추상 인터페이스로 여러 백엔드를 지원하고 플러그인 메커니즘으로 새 언어 모델, 임베딩 모델, 저장 백엔드를 유연하게 통합합니다. 기본 버전 외에도 그래프 메모리 변형인 **Mem0-g**를 제공합니다. 메모리를 독립적인 사실 항목이 아니라 엔터티-관계 그래프로 나타내어 메모리 사이의 관계 구조를 명시적으로 포착합니다. 이를 통해 다단계 및 시간 문제의 성능을 높입니다. 그래프 구조의 지식 표현은 이 장 뒤의 GraphRAG 절에서 자세히 설명합니다.
