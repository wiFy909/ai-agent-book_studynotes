...
| 📚 10 章系统正文 | 🧪 94 个配套实验 | 🈶 中文与 English 正文 |
| :---: | :---: | :---: |
...
| 🧭 实践反思 | 用自己的 Agent 与 Vibe Coding 经历相互印证 | 个人解读、反例和后续问题 |
| 📝 复盘 | 判断适用条件与可迁移性 | 行动项和阶段结论 |
...
2. **实践代码**：能运行时记录真实环境、命令和观察；尚未运行时明确说明尚未执行。
3. **实践反思**：区分原作事实、个人理解、推演和未知，不把感受写成书中结论。
4. **记录结论**：写明已经观察到的结果、尚未验证的部分和下一步问题。
...
**最后更新：2026-07-31 · 当前阶段：第 1 章《Agent 基础知识》——完成首次实机实验复现，继续精读**

| 进度项 | 当前状态 | 直达入口 |
| --- | --- | --- |
| 章节 | 第 1 / 10 章，完成首次实机实验复现；第 1 章精读仍在继续 | [第 1 章正文](book/chapter1.md) |
...
| 日期 | 阶段 | 本次变化 | 心得文件 |
| --- | --- | --- | --- |
| 2026-07-31 | 第 1 章推进 | 新增模型选择与 Agent 安全两组笔记；完成 Experiment 1-1 的 Kimi K3 实机复现，并记录上下文消融的正确性、耗时和结论边界 | [学习笔记](studydocs.md) · [实验复现记录](experiments-replication.md) |
| 2026-07-30 | 第 1 章进行中 | 建立精读 fork 首页；整理 7 组主题；补充 RL、神经网络与工具调用的边界说明；同步上游最新的 94 个实验和 20 个外部仓库信息 | [`studydocs.md`](studydocs.md) |
